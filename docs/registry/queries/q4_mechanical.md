...
| `mtafiti.perception.pinned_model_provenance` | `PROM-9-2a-real-worker-provenance`, `PROM-S1-honesty-grammar-source-labels` | yes | [CLIENT-PROMISE · UNVERIFIED · ESCALATE-AT-CLOSE] | — | enforcement field 'Contract field required (model_id + weights_sha)' lacks testable telemetry/gate reference |
...
| `akki.frontend.response_class_taxonomy_four_visual` | `PROM-S1-refusal-taxonomy-closed` | yes | [CLIENT-PROMISE · UNVERIFIED · ESCALATE-AT-CLOSE] | — | enforcement field 'Jest (test: each class has a distinct accent color + distinct test-id + classifyResponse mapping)' lacks testable telemetry/gate reference |
| `akki.backend.wizard_freeze_refuses_when_lawful_basis_empty` | `PROM-S1-refusal-taxonomy-closed` | yes | [CLIENT-PROMISE · UNVERIFIED · ESCALATE-AT-CLOSE] | — | enforcement field 'pytest (pre-existing wizard tests)' lacks testable telemetry/gate reference |
...
| `akki.frontend.open_copy_slot_marker` | enforcement field 'Jest (renders marker + slot name + dashed sage style)' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
| `akki.frontend.class_with_claim_headline` | enforcement field 'Jest (test-ids `refusal-class-with-claim`, `acl-class-with-claim`, `validation-class-with-claim`, `infra-class-with-claim`)' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
| `akki.connect.new_source_stub_seam_honestly_marked` | enforcement field 'integration (GovernedRefusalCard renders on submit)' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
| `akki.commission_view.milestone_front_page_landed` | enforcement field 'integration + manual UI (renders test-ids `commission-milestone-front-page` + `milestone-list` + `spend-vs-quote-row`)' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
| `akki.commission_view.reads_only_existing_artifacts` | enforcement field 'manual (endpoint inventory — no new commission-view backend route)' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
| `akki.commission_view.missed_milestone_renders_as_plainly_as_done` | enforcement field 'inspection (STATUS_STYLE map + Jest snapshot pending)' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
| `akki.commission_view.drill_down_evidence_never_leads` | enforcement field 'inspection (DOM order)' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
| `akki.backend.wizard_state_carries_milestones_via_sidecar` | enforcement field 'pytest + parity 34/34 unchanged' lacks testable telemetry/gate reference | overlaps: CL-1.3 |
