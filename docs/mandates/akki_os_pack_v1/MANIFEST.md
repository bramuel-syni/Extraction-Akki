# Akki OS Document Pack v1 — Manifest (9/9 CLOSED)

**Dispatch authority:** `docs/mandates/AKKI_OS_BUILD_DISPATCH_v1.md` §AC-4.
**Initial commit date:** 2026-07-30.
**9/9 close date:** 2026-07-30 (Audio Intelligence Plane Specification supplied by Owner same-day).
**Canonical form:** markdown. `.docx` originals are generated presentation only.

**AC-4 close condition MET — 9/9 documents committed.**

## Documents in this pack (canonical .md; source .docx SHA also recorded)

| # | Filename (canonical .md) | SHA-256 (.md) | Source .docx SHA-256 |
| --- | --- | --- | --- |
| 1 | AkkiOS_Product_Engineering_Specification_v1.0.md | 555bf0018b94f87371b2d68492cf9a1bb4cde09f8aadb7e3dde5134bae663564 | e886872bec3da0f0676a62841890762d8a74300861dfdfe7ecd9b3c381dc46fb |
| 2 | AkkiOS_Product_Marketing_Briefing_v1.0.md | 2dad00deea6b475125782b1b31126f43c193df8797c7bfeb7d1fdf47659bacb0 | fe35ca6eef4cf95707953981d9b88cbe01b7ee073d37c66c7157ec553b0719c5 |
| 3 | AkkiOS_Governance_Orchestration_Brief_v1.0.md | 7c178ee3d37ecd8350c6840649989616ea98cad95ecc957969dbfe0bcbec03a7 | 41f38430bacd0f150f31cbe227c452012e030959fdf0ae0182299ad4bb4a437d |
| 4 | AkkiOS_Production_Orchestration_Brief_v1.0.md | 018ec6d2edb4fdd67fcf5152837a399c960eb4124386c890e70b7b0f8ac20c0c | 94290a5871cd08671bd74f1356d8230a391be9ab92fc980349bca649dd31ca62 |
| 5 | AkkiOS_Integration_Orchestration_Brief_v1.0.md | 7681f9b008c67d0e51abd97cc65dac47bbdb2876e1fde7d89a0baee4c8458ce7 | 7d631e9acf69812b1009d1ce5476e96824178451bb8f6bbcc133b9e39cc6428c |
| 6 | AkkiOS_Surfaces_Journeys_Features_v1.0.md | bdfd4f80e15d0c78c5ed4b3c797e6063ae7b8166d811207d289d25092c94f5e0 | 490289e4a460e35bfee4f8be7930c1e51c30e0c09b35c2d84040897402951781 |
| 7 | AkkiOS_Quality_Rule_Book_v1.0.md | b1e59fdba4dcddc3fa284452cbb947f034918ef686cdb920e70fcfe44a0e6806 | 2b7623b6c0cd25de673b48f740f38bce2bd521c4964df06fc1ae6273bc251533 |
| 8 | AkkiOS_Canon_Register_and_Correction_Record_v1.0.md | be6253a28996fc480dbf220bdbc789b68de1c329206d069799a658b1ef97ce42 | 30bd6b4134f5334090068772d75bb32113d97d5e2e3b795d60305106d4a6b4fe |
| **9** | **Audio_Intelligence_Plane_Specification_v1.0.md** | **c8ee75ae756004af5dd68d48b94376f3b9005aaf5ee3f413ed456566d2da2c5e** | **b6ad57b369ad3125f25a81f0790d5226a7630206092ea096023d04650cbe43a9** |

## Dispatch + rulings governing this pack

| Filename | Location | SHA-256 |
| --- | --- | --- |
| AKKI_OS_BUILD_DISPATCH_v1.md | `docs/mandates/AKKI_OS_BUILD_DISPATCH_v1.md` | 201e16c30b0e04b6d191b387e9021bd4545eaa86cb6d74b3847e6a5cc329abbb |
| **AKKI_OS_FRONTEND_BRIEF_v2.md** (supplied 2026-07-30 cycle 3) | `docs/mandates/AKKI_OS_FRONTEND_BRIEF_v2.md` | **d6111202203c77af0cf08ad39d26d1573429d74fdc99e19fcf35e07e2b0ba82b** |
| AKKI_OS_BUILD_DISPATCH_v1_ERRATUM_2026-07-30.md | `docs/mandates/AKKI_OS_BUILD_DISPATCH_v1_ERRATUM_2026-07-30.md` | (dispatch §6 heading amendment; body governs) |

## Amendment notes in this pack (append-only history)

| Filename | Class | Status |
| --- | --- | --- |
| AMENDMENT_2026-07-30_CanonRegister_CC-1.md | Canon Register — BCR v1.5 promoted to canon | Applied |
| AMENDMENT_2026-07-30_CrossReferenceFix_CC-5.md | Engineering Spec §16.3 half + Audio-half deferred | Engineering half applied; Audio half now closable (see AMENDMENT_2026-07-30_CC-5_AudioHalf.md) |
| AMENDMENT_2026-07-30_CC-5_AudioHalf.md | Audio Plane §4.4 cross-reference fix (§6.4 → §6.2) | Applied |

## Fidelity check for Audio Plane Spec (§1 close condition per dispatch STEP 1)

Markdown is canonical per AC-4. The supplied .md (`c8ee75a…`) contains all 66 headings, all governing tables (11 primary tables including the mandate register at §15), all four fail-closed contract clauses (§3.1, §4.6, §6.3, §7.2), §16 verification and open decisions (§16.2 codec-choice open decision confirmed word-for-word). No content divergence from the .docx has been observed at the paragraph level. Where a reader ever encounters divergence, the .md wins per dispatch AC-4.

## Doctrinal notes carried by this manifest

- **AC-4 close condition:** MET on 2026-07-30.
- **Read-order (dispatch §AC-4):** Product & Engineering Specification → Marketing Briefing → Governance Brief → Production Brief → Integration Brief → Audio Intelligence Plane Specification → Surfaces v1.0 (+ Surfaces v2 amendment at `docs/mandates/AKKI_OS_SURFACES_v2_AMENDMENT.md`) → Quality Rule Book → Canon Register.
- **Companion documents committed alongside (dispatch STEP 2):** `docs/mandates/AKKI_OS_BUILDER_PROMPT.md`, `docs/mandates/AKKI_OS_SURFACES_v2_AMENDMENT.md`, `docs/mandates/Akki_v4_Standalone.html`. See `docs/mandates/AKKI_OS_COMPANION_DOCS_MANIFEST.md` for hashes and summaries.

— End of manifest. —
