...
- **§4.1 Landing (one page, no tabs · exact five-section DOM order):** headline slot with same-testid across pre-connection + steady-state (layout invariant) · status banner with signer + deployment target + primary regulator + read-only link to /instance/config · three cards (connections healthy/total · last sync · egress posture with dormant-honest marker) · record table (Source · Protocol · Cadence · State ∈ {connected · in progress · awaiting credentials · failed}; entire row clickable → /connect/source/:id) · footer with `connect-footer-govern-link` → /govern/rules. **Gate: no governance content on this page** — Connect LINKS, never duplicates.
- **§4.2 Seven Connect rules:** 6 non-ceiling rules rendered honestly from backend rulebook (rule 5 egress-posture is dormant-marked); **rule 7 commission auto-run ceiling $1,000 · ∞ permitted · single-source-of-truth read from `services/connect/rulebook.py::get_effective_auto_run_ceiling_usd`**. Direct-write refused with route to /govern/change-rule (Gate `gate_auto_run_ceiling_1000_change_a_rule_only`).
- **§4.2 A5 Class-D registry declaration:** at /connect/setup (master_admin only). Declared registries live EMPTY + FAIL-CLOSED until first /govern/registries upload; chips on Connect home link into /govern/registries.
- **§4.4 Source profile:** answer-first mapping header "N of M fields confirmed · K need attention"; plain-language questions with resolution controls; full field list collapsed behind toggle; operator/master_admin/admin see Connect·Test·Retry; other classes read-only with who-confirmed + when.
- **Role gates enforced server-side:** master_admin adds source; operator/master_admin/admin connect/test/retry; analyst gets 403 auth_scope_insufficient on POST.
- **EE-R4 no parallel mechanism (Owner binding 2a):** `commission_verdict_engine::commit_commission` awaits `get_effective_auto_run_ceiling_usd` at commit time — SAME seam Change-a-Rule operates on. Gate `test_c_b5_ceiling_read_from_single_source_of_truth` proves: insert an `effective` row → both /connect/rules and /use_data/ceiling reflect the new value.
- **QRB DEFAULT markers (Owner binding 3a):** instance-config seam extended with `deployment_target`, `primary_regulator`, `credentials_holder`, `defaults[]`. Landing banner renders visible "default — not yet confirmed" chip on each defaulted field.
- **Sample fixtures per identity (Owner binding 1a):** 4 sources per identity (connected/in_progress/awaiting_credentials/failed) all SAMPLE-marked; failed state carries honest plain-language TLS handshake failure reason (never a bare "failed"). Plus 2 declared Class-D registries (sanctioned_partners · restricted_terms). Idempotent across restart.
- **SYSTEMIC gate expanded:** 5 fixture-capable surfaces now registered (Use Data pipeline · Govern record · Govern holds · Connect landing · Connect setup). Future sub-cycles must extend the registry.
- **UI-1-B micro-follow-up rolled in:** `/api/govern/trust_center_record` sort now pins `is_sample DESC` primary so seeded fixtures always surface even as collections grow. Admin sees 2 rule-change SAMPLE badges on `/govern`.
- **Existing /connect stub pages:** salvaged where they match (retained `/api/connect/capabilities` dormant inventory), rebuilt where they didn't (five-section landing).
- **Regression:** Backend pytest **1529 pass · 2 skip · 0 fail** (+11 UI-1-C invariants). Frontend Jest **17 suites · 139 pass · 3 skipped-to-salvage · 0 fail** (+10 UI-1-C cells +5-surface systemic gate). Parity **36/36 held constant** throughout. Retired-vocab gate re-runs green.
- **Live-preview iter20 verdict:** all 8 integration cells GREEN; `retest_needed: false`. Post-iter20 minor design fix (row-click widened to full row).
- **Close report:** `/app/docs/rulings/ui1c_close_report_2026-08-02.md` (11 sections + FPR rows + WHAT-TO-LOOK-AT walk-through).

### UI-1-B iter17-19 close (2026-08-01)
